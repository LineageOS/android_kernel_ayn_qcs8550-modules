...
static int fts_ft5452j_fwupg_get_boot_state(struct fts_upgrade *upg,
	enum FW_STATUS *fw_sts)
{
	...
	ret = fts_read(upg->fts_data, cmd, cmd_len, val, 3);
	if (ret < 0) {
...
static bool fts_ft5452j_fwupg_check_state(struct fts_upgrade *upg,
	enum FW_STATUS rstate)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		ret = fts_ft5452j_fwupg_get_boot_state(upg, &cstate);
...
***********************************************************************/
static int fts_ft5452j_fwupg_reset_to_romboot(struct fts_upgrade *upg)
{
	int ret = 0;
	int i = 0;
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		ret = fts_ft5452j_fwupg_get_boot_state(upg, &state);
...
static int fts_ft5452j_pram_ecc_cal_algo(struct fts_upgrade *upg,
	u32 start_addr,
	u32 ecc_length)
{
	int ret = 0;
	...
	cmd[7] = 0xCC;
	ret = fts_write(upg->fts_data, cmd, 8);
	if (ret < 0) {
...
static int fts_ft5452j_pram_ecc_cal(struct fts_upgrade *upg, u32 saddr, u32 len)
{

	return fts_ft5452j_pram_ecc_cal_algo(upg, saddr, len);
}

static int fts_ft5452j_pram_write_buf(struct fts_upgrade *upg, u8 *buf, u32 len)
{
	int ret = 0;
	u32 i = 0;
	u32 j = 0;
...
static int fts_ft5452j_pram_start(struct fts_upgrade *upg)
{
...
static int fts_ft5452j_pram_write_remap(struct fts_upgrade *upg)
{
	int ret = 0;
	int ecc_in_host = 0;
	int ecc_in_tp = 0;
	...
	ecc_in_host = fts_ft5452j_pram_write_buf(upg, pb_buf, pb_len);
	if (ecc_in_host < 0) {
	...
	/* read out checksum */
	ecc_in_tp = fts_ft5452j_pram_ecc_cal(upg, 0, pb_len);
	if (ecc_in_tp < 0) {
		FTS_ERROR( "read pramboot ecc fail");
		return ecc_in_tp;
	}
	...
	/*start pram*/
	ret = fts_ft5452j_pram_start(upg);
	if (ret < 0) {
...
static int fts_ft5452j_pram_write_init(struct fts_upgrade *upg)
{
	int ret = 0;
	bool state = 0;
	...
	ret = fts_ft5452j_fwupg_get_boot_state(upg, &status);
	if (status != FTS_RUN_IN_ROM) {
		...
		ret = fts_ft5452j_fwupg_reset_to_romboot(upg);
		if (ret < 0) {
			FTS_ERROR("reset to romboot fail");
			return ret;
		}
	...
	ret = fts_ft5452j_pram_write_remap(upg);
	if (ret < 0) {
	...
	state = fts_ft5452j_fwupg_check_state(upg, FTS_RUN_IN_PRAM);
	if (!state) {
...
int ft5452j_write_pramboot_private(struct fts_upgrade *upg)
{
	int ret = 0;

	ret = fts_ft5452j_pram_write_init(upg);
	if (ret < 0) {
...
* Return: return 0 if success, otherwise return error code
***********************************************************************/
static int fts_ft5452j_upgrade(struct fts_upgrade *upg, u8 *buf, u32 len)
{
	int ret = 0;
	u32 start_addr = 0;
	u8 cmd[4] = { 0 };
